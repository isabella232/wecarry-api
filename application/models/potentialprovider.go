...
// FindByRequestID gets the PotentialProviders associated with the Request.
...
// If the currentUser is the requester or a SuperAdmin, then all potentialProviders are included.
// If the currentUser is one of the potentialProviders, that one potentialProvider is included.
// Otherwise, no potentialProviders are included.
func (p *PotentialProviders) FindByRequestID(request Request, currentUser User) error {
	if request.ID <= 0 {
		return fmt.Errorf("error finding potential_provider, invalid id %v", request.ID)
	}
	...
	if err := DB.Eager("User").Where(whereQ).All(p); err != nil {
		if domain.IsOtherThanNoRows(err) {
			return fmt.Errorf("failed to find potential_provider records for request %d, %s",
				request.ID, err)
		}
	}
