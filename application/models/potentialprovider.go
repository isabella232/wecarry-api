...
// FindByPostID gets the Users associated with the Post's PotentialProviders
...
func (p *PotentialProviders) FindByPostID(post Post, currentUser User) error {
	if post.ID <= 0 {
		return fmt.Errorf("error finding potential_provider, invalid id %v", post.ID)
	}
	...
	if err := DB.Eager("User").Where(whereQ).All(p); err != nil {
		if domain.IsOtherThanNoRows(err) {
			return fmt.Errorf("failed to find potential_provider records for post %d, %s",
				post.ID, err)
		}
	}
