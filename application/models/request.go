...
// GetPotentialProviders returns the PotentialProvider objects associated with the Request's
// PotentialProviders
func (r Request) GetPotentialProviders(currentUser User) (PotentialProviders, error) {
	if r.ID <= 0 {
		return PotentialProviders{}, errors.New("cannot get PotentialProviders for empty Request")
	}

	providers := PotentialProviders{}

	if err := providers.FindByRequestID(r, currentUser); err != nil {
		return PotentialProviders{}, fmt.Errorf("failed to find potential_provider record for request %d, %s",
			r.ID, err)
	}
...
// touched.
func (r Requests) FilterDestination(location Location) Requests {
	filtered := make(Requests, 0)
	_ = DB.Load(&r, "Destination")
	for i := range r {
...
func (r Requests) FilterOrigin(location Location) Requests {
	filtered := make(Requests, 0)
	_ = DB.Load(&r, "Origin")
	for i := range r {
